...
class Menu
{
    ...
    public static function export($name)
    {
        ...
        if ($menu) {
            $ruleList = collect(AuthRule::where('id', 'in', $ids)->select())->toArray();
    ...
    public static function getAuthRuleIdsByName($name)
    {
        ...
        if ($menu) {
            // 必须将结果集转换为数组
            $ruleList = collect(AuthRule::order('weigh', 'desc')->field('id,pid,name')->select())->toArray();
            // 构造菜单数据
