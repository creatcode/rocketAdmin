        ...
        try {
            ...
            foreach ($columnList as $k => $v) {
                ...
                if ($v['DATA_TYPE'] != 'text' && $inputType != 'fieldlist') {
                    ...
                    if ($this->headingFilterField && $this->headingFilterField == $field && $itemArr) {
                        ...
                        $multipleHtml = $this->getReplacedStub('html/multiple-html', ['field' => $field, 'fieldName' => parse_name($field, 1, false), 'controllerUrl' => $controllerUrl]);
                    }
            ...
            if ($relations) {
                ...
                foreach ($relations as $index => $relation) {
                    ...
                    //构造关联模型的方法
                    $relationMethodList[] = $this->getReplacedStub('mixins' . DIRECTORY_SEPARATOR . 'modelrelationmethod' . ($relation['relationMode'] == 'hasMany' ? '-hasmany' : ''), $relation);
