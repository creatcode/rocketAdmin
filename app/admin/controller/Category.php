...
class Category extends Backend
{
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            foreach ($this->categorylist as $k => $v) {
                $typeMatched = $type == "all" || $type == null || $v['type'] == $type;
                if ($search) {
                    $nameMatched = stripos($v['name'], $search) !== false || stripos($v['nickname'], $search) !== false;
        ...
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                if ($params['pid'] != $row['pid']) {
                    $childrenIds = Tree::instance()->init(collect(\app\common\model\Category::select())->toArray())->getChildrenIds($row['id'], true);
