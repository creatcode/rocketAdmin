...
class AdminAuth extends AuthService
{
    protected $_error = '';
    ...
    public function logout()
    {
        ...
        setcookie('fastadmin_userinfo', '', $_SERVER['REQUEST_TIME'] - 3600, rtrim((string)url("/" . app('http')->getName(), [], false), '/'));
    ...
    public function getBreadCrumb($path = '')
    {
        ...
        foreach ($this->rules as $rule) {
            if (isset($pathArr[$rule['name']])) {
                ...
                $rule['url'] = (string)url($rule['name'], [], false);
    ...
    public function getSidebar($params = [], $fixedPage = 'dashboard')
    {
        ...
        // 必须将结果集转换为数组
        $ruleList = collect(AuthRule::where('status', 'normal')
            ->where('ismenu', 1)
            ->order('weigh', 'desc')
            ->cache("__menu__")
            ->select())->toArray();
